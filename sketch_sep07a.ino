...
public:
    void initializeDevice(String deviceName, String devicePass) {
      display.displayText("|Connecting|", 20);
        ...
        SerialBT.setPin(devicePass.c_str());
    ...
    void DisplayUpdatedValue() {
        ...
        if (myELM327.nb_rx_state == ELM_SUCCESS) {
            display.displayText(pidController.getCurrentValString(), 12); 
            display.displayText(String(pidController.getCurrentVal()), 20);
        } else if (myELM327.nb_rx_state != ELM_GETTING_MSG) {
            myELM327.printError();    
        }
    }
...
void setup()
{
    device.initializeDevice("vLinker MC-Android", "1234");
}
