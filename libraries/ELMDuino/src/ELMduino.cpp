...
{
	uint64_t result = (response << ((numPayChars - numExpectedBytes) * 8)) >> ((numPayChars - numExpectedBytes) * 8);
	return (result * scaleFactor) + bias;
}

uint32_t ELM327::conditionResponseInt(const uint64_t &response, const uint8_t &numExpectedBytes, const float &scaleFactor, const float &bias)
{
	uint64_t result = (response << ((numPayChars - numExpectedBytes) * 8)) >> ((numPayChars - numExpectedBytes) * 8);
	return result;
}
...
float ELM327::processPID(const uint8_t& service, const uint16_t& pid, const uint8_t& num_responses, const uint8_t& numExpectedBytes, const float& scaleFactor, const float& bias)
{
	...
	else if (nb_query_state == WAITING_RESP)
	{
		...
		if (nb_rx_state == ELM_SUCCESS)
		{
			...
			uint32_t result = conditionResponse(findResponse(), numExpectedBytes, scaleFactor, bias);
			// Serial.print("Result in process PID is : ");
...
uint32_t ELM327::processPIDInt(const uint8_t& service, const uint16_t& pid, const uint8_t& num_responses, const uint8_t& numExpectedBytes, const float& scaleFactor, const float& bias)
{
	if (nb_query_state == SEND_COMMAND)
	{
		queryPID(service, pid, num_responses);
	...
	else if (nb_query_state == WAITING_RESP)
	{
		...
		{
			nb_query_state = SEND_COMMAND; // Reset the query state machine for next command
			uint32_t result = conditionResponseInt(findResponse(), numExpectedBytes, scaleFactor, bias);
			Serial.print("Result in process PID is : ");
			Serial.println(result);
			return conditionResponseInt(findResponse(), numExpectedBytes, scaleFactor, bias);
		}
		else if (nb_rx_state != ELM_GETTING_MSG)
			nb_query_state = SEND_COMMAND; // Error or timeout, so reset the query state machine for next command
	}
...
uint32_t ELM327::supportedPIDs_1_20()
{
	return (uint32_t)processPIDInt(SERVICE_01, SUPPORTED_PIDS_1_20, 1, 4);
}
...
uint32_t ELM327::supportedPIDs_21_40()
{
	return (uint32_t)processPIDInt(SERVICE_01, SUPPORTED_PIDS_21_40, 1, 4);
}
...
uint32_t ELM327::supportedPIDs_41_60()
{
	return (uint32_t)processPIDInt(SERVICE_01, SUPPORTED_PIDS_41_60, 1, 4);
}
...
uint32_t ELM327::supportedPIDs_61_80()
{
	return (uint32_t)processPIDInt(SERVICE_01, SUPPORTED_PIDS_61_80, 1, 4);
}
